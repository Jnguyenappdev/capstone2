...
function getDetailMountainChange() {
  ...
  const mountainNameDisplay = mountainsArray.find(
    (mountain) => mountain.name === selectMountainValue
  );

  if (mountainNameDisplay.name != "Select Mountain...") {
    ...
    otherEl.innerHTML =
      "Effort:" +
      mountainNameDisplay.effort +
      "<br>" +
      "Coordinates:" +
      mountainNameDisplay.coords.lat +
      " " +
      mountainNameDisplay.coords.lng;
    ...
    //removes info when select one is selected again
    const mountainNameEl = document.getElementById("mountainName");
    mountainNameEl.innerHTML = "";
